...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query database for featured projects (where "Featured" or "Show on Homepage" is true)
    const databaseId = process.env.NOTION_PROJECTS_DB_ID || ''
    const apiKey = process.env.NOTION_API_KEY || ''
    
    // First, try to get all pages and filter client-side (more flexible)
    // This handles cases where the checkbox property might not exist yet
    let queryResponse
    let pages: any[] = []
    
    try {
      // Try with filter first
      queryResponse = await fetch(`https://api.notion.com/v1/databases/${databaseId}/query`, {
        method: 'POST',
        headers: {
      ...
      if (queryResponse.ok) {
        ...
        console.log('Filter query failed, trying to get all pages...')
        ...
        if (allPagesResponse.ok) {
          ...
          console.error('Database query failed:', allPagesResponse.status, errorText)
          return NextResponse.json({ 
            projects: [],
      ...
      console.error('Error fetching from Notion:', fetchError)
      return NextResponse.json({ 
        projects: [],
    ...
    const projects = pages.map((page: any) => {
      const props = page.properties || {}
    ...
    }).filter((project: any) => project.slug) // Only include projects with a slug
